...
class QrReader {
    ...
    QrReader(int width, int height, Activity context, FirebaseVisionBarcodeDetectorOptions options,
        ...
        if (android.os.Build.VERSION.SDK_INT >= 21) {
            ...
            qrCamera = new QrCameraC2(width, height, texture, context, new QrDetector(communicator, context, options));
        } else {
            Log.i(TAG, "Using old camera API.");
            qrCamera = new QrCameraC1(width, height, texture, context, new QrDetector(communicator, context, options));
        }
    }
